...
export default function EditOpportunity({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Opportunity Update Failed:", err, err?.fieldErrors, err?.message);
      setError(err instanceof ApiError ? err.message : "Failed to update opportunity. Please check console logs.");
      document.querySelector('main')?.scrollTo({ top: 0, behavior: 'smooth' });
    } finally {
      setSaving(false);
    }
